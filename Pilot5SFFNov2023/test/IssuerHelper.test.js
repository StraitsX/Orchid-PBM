...
describe("IssuerHelper", function () {
  beforeEach(async function () {
    ...
    this.issuerHelper = await deploy("IssuerHelper", this.pbm.address, this.minimalForwarder.address);
    ...
    await this.pbm.connect(deployer).initialise(this.erc20Token.address, targetEpoch + 100000000, this.addressList.address);
    await this.pbm.connect(deployer).createPBMTokenType("testPBMenvelope", 5, targetEpoch, deployer.address, "uri", "expired_uri");
  });
  ...
  describe("meta txn", function () {
    ...
    it("should call the processLoadAndSafeTransfer thru a metatxn", async function () {
      const { pbm, addressList, erc20Token, issuerHelper, minimalForwarder } = this;
      ...
      await addressList.connect(deployer).addMerchantAddresses([other.address], 'other');
      ...
      const { request, signature } = await signMetaTxRequest(wallet.provider, minimalForwarder, {
        from: wallet.address,
        to: issuerHelper.address,
        data: issuerHelper.interface.encodeFunctionData('processLoadAndSafeTransferFrom', [erc20Token.address, wallet.address, other.address, 0, ethers.utils.parseUnits("50", 18)]),
      });
      ...
      expect(finalOtherBalance.sub(initialOtherBalance)).to.equal(ethers.utils.parseUnits("50", 18));
  ...
  describe("processLoadAndSafeTransfer", function () {
    it("should transfer tokens from wallet to IssuerHelper and call loadAndSafeTransfer on PBM", async function () {
      ...
      txn = await issuerHelper.connect(wallet).processLoadAndSafeTransferFrom(erc20Token.address, wallet.address, other.address, 0, ethers.utils.parseUnits("50", 18));
    ...
    it("should fail if wallet is not whitelisted", async function () {
      ...
      await expect(
        issuerHelper.connect(whitelister).processLoadAndSafeTransferFrom(erc20Token.address, wallet.address, other.address, 0, ethers.utils.parseUnits("50", 18))
