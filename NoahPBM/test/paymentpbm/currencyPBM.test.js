...
describe("Noah Payment Manager Test", () => {
    ...
    describe("Noah PBM Core Test", () => {
        ...
        it("Should ensure payments can only be created if enough funding for ali use case", async () => {
            ...
            await expect(
                pbm.connect(aliOperator).requestPayment(aliOmnibus.address, merchant.address, 0, 1000, "unique_payment_id", "0x")
        ...
        it("Should ensure only payment manager can call revertPayment on CurrencyPBM", async () => {
            ...
            await expect(pbm.connect(aliOmnibus).revertPaymentForCancel(aliOmnibus.address, "unique_payment_id")).to.be.revertedWith(
                "PBM: Only noah payment manager can revert payment"
            );

            await expect(pbm.connect(aliOmnibus).revertPaymentForRefund(aliOmnibus.address, "unique_payment_id", 500)).to.be.revertedWith(
                "PBM: Only noah payment manager can revert payment"
            );
        });
        ...
        it("Should ensure refund amount is no more than payment amount when refund a payment", async () => {
            ...
            await expect(noahPaymentManager
                .connect(noahCrawler)
                .refundPayment(aliOmnibus.address, "unique_payment_id", "refund_unique_id", parseUnits("600", await xsgdToken.decimals()), "0x")).to.be.revertedWith("Refund value must be less than or equal to remaining paid value");
        ...
        it("Should ensure second partial refund amount is no more than remaining paid amount when partial refund a payment", async () => {
            ...
            await noahPaymentManager
                .connect(noahCrawler)
                .refundPayment(aliOmnibus.address, "unique_payment_id", "refund_unique_id", parseUnits("200", await xsgdToken.decimals()), "0x");
            ...
            await expect(noahPaymentManager
                .connect(noahCrawler)
                .refundPayment(aliOmnibus.address, "unique_payment_id", "refund_unique_id", parseUnits("400", await xsgdToken.decimals()), "0x")).to.be.revertedWith("Refund value must be less than or equal to remaining paid value");
        ...
        it("Should ensure a partial refund payment is updated to REFUNDED and correct amount of PBMToken minted back to from address when refund made the full amount", async () => {
            ...
            await noahPaymentManager
                .connect(noahCrawler)
                .refundPayment(aliOmnibus.address, "unique_payment_id", "refund_unique_id", parseUnits("300", await xsgdToken.decimals()), "0x");
        ...
        it("Should ensure a fully refund payment is updated to REFUNDED and PBMToken minted back to from address when fully refund a payment", async () => {
            ...
            await noahPaymentManager
                .connect(noahCrawler)
                .refundPayment(aliOmnibus.address, "unique_payment_id", "refund_unique_id", parseUnits("500", await xsgdToken.decimals()), "0x");
