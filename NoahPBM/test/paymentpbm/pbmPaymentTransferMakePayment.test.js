...
describe("PBMPayment Test", async () => {
  ...
  beforeEach(async () => {
    ...
    // Create address list
    await whilteListMerchant(addressList, [merchant1.address, merchant2.address, merchant3.address]);
  });
  ...
  describe("PBM Payment Payment and Transfers", async () => {
    ...
    it("transfer XSGD PBM to non merchant successfully", async () => {
      ...
      const nonMerchantBeforeBalance = await pbm.balanceOf(nonMerchant.address, 0);
      ...
      // Perform transfer
      await pbm.safeTransferFrom(owner.address, nonMerchant.address, 0, 1, "0x");

      // Check balances after transfer
      const nonMerchantAfterBalance = await pbm.balanceOf(nonMerchant.address, 0);
    ...
    describe("PBM Payment via safeTransfer Test", async () => {
      it("transfer XSGD PBM and unwrap payment to merchant successfully", async () => {
        // Check balances before transfer
        const MerchantBeforeSpotBalance = await xsgdToken.balanceOf(merchant3.address);
        ...
        // Check balances after transfer
        const MerchantAfterSpotBalance = await xsgdToken.balanceOf(merchant3.address);
      ...
      it("transfer of PBM should fail if token holder does not have pbm", async () => {
        expect(pbm.safeTransferFrom(nonMerchant.address, merchant3.address, 0, 1, "0x")).to.be.revertedWith(
      ...
      it("Should allow the combination of different PBM types with metadata being emitted", async () => {
        ...
        encodedMetadata = ethers.utils.hexlify(ethers.utils.toUtf8Bytes(metadataString));
