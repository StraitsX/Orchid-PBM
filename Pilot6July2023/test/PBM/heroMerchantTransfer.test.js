...
describe('PBM', () => {
  ...
  describe('PBM transfer test', () => {
    ...
    it('transfer to hero merchant successfully mint heroNFT to user', async () => {
      ...
      expect(heroMerchantAfterXsgdBalance).to.be.equal(
        parseUnits('1', await xsgdToken.decimals()),
      );
    ...
    it('transfer to hero merchant successfully will not mint heroNFT if user already have one', async () => {
      ...
      expect(heroMerchantFinalXsgdBalance).to.be.equal(
        parseUnits('2', await xsgdToken.decimals()),
      );
    ...
    it('transfer to hero merchant successfully will not mint heroNFT if user received it once', async () => {
      ...
      await heroNFT.safeTransferFrom(
        owner.address,
        nonMerchant.address,
        2,
        1,
      ...
      expect(await xsgdToken.balanceOf(merchant2.address)).to.be.equal(
        parseUnits('3', await xsgdToken.decimals()),
      );
    });
    ...
    it('batch Transfer to hero merchant will not mint heroNFT if user received it once', async () => {
      ...
      expect(await xsgdToken.balanceOf(merchant1.address)).to.be.equal(
        parseUnits('9', await xsgdToken.decimals()),
      );
    });
  });
});
